...
const Users = () => {
  ...
  useEffect(() => {
    console.log('Usuario actual en Users:', loggedUser);
    loadUsers();
  }, [loggedUser]);
  ...
  const confirmDelete = async () => {
    if (userToDelete) {
      try {
        ...
        console.log('Intentando toggle status para usuario:', backendUserId, userToDelete);
        ...
        console.error('Error completo al toggle user status:', err);
        setError('Error al cambiar el estado del usuario: ' + (err.message || 'Error desconocido'));
    ...
    <div className="bg-gradient-page min-h-screen">
                          ...
                          <div className="flex gap-0.5">
                            ...
                            {loggedUser && loggedUser.role === 'admin' && (
                              <button
                                ...
                                className={`btn ${user.isActive ? 'btn-danger' : 'btn-success'} text-xs`}
                                style={{ padding: '0.4rem 0.5rem' }}
      ...
      {showDeleteModal && userToDelete && (
        <div className="modal-overlay" onClick={closeDeleteModal}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="modal-title">Confirmar {userToDelete.isActive ? 'desactivación' : 'activación'}</h3>
              <button onClick={closeDeleteModal} className="modal-close">×</button>
            </div>
            <div className="modal-body" style={{ padding: '2rem' }}>
              <p className="mb-4">¿Estás seguro de que deseas {userToDelete.isActive ? 'desactivar' : 'activar'} al usuario <strong>{userToDelete.username}</strong>?</p>
            ...
            <div className="modal-footer">
              ...
              <button onClick={confirmDelete} className={`btn ${userToDelete.isActive ? 'btn-danger' : 'btn-success'}`}>
                {userToDelete.isActive ? 'Desactivar' : 'Activar'}
              </button>
            </div>
          </div>
        </div>
